...
# ✔ CORREÇÃO: zeros automáticos no valor
def mascara_valor(valor_digitado):
    numeros = ''.join(filter(str.isdigit, valor_digitado))
    ...
    if not df.empty:
        ...
        with col_b1:
            if st.button("Salvar edição", key="btn_salvar_edicao"):
                df.loc[df["id"] == id_atual, "descricao"] = nova_descricao
                df.loc[df["id"] == id_atual, "categoria"] = nova_categoria
                df.loc[df["id"] == id_atual, "forma_pagamento"] = nova_forma
                df.loc[df["id"] == id_atual, "operadora"] = nova_operadora
                df.loc[df["id"] == id_atual, "tipo"] = novo_tipo
                df.loc[df["id"] == id_atual, "valor"] = novo_valor
                df.loc[df["id"] == id_atual, "vencimento"] = novo_vencimento

                salvar_dados(df)
                st.success("Todas as parcelas foram atualizadas!")

        with col_b2:
            if st.button("Excluir lançamento", key="btn_excluir"):
                df = df[df["id"] != id_atual]
                salvar_dados(df)
                st.success("Todas as parcelas foram excluídas!")
    else:
